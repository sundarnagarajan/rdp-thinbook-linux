...
if [ -n "$REMASTER_CMDS_DIR" ]; then
    run_remaster_commands "$EXTRACT_DIR" "${REMASTER_CMDS_DIR}" || exit 1
else
    echo "No REMASTER_CMDS_DIR. Not running any remaster commands"
fi

update_squashfs "$EXTRACT_DIR" "$SQUASHFS_PATH" "${MANIFEST_PATH}" "${SIZE_FILE}" || exit 1
update_iso "$EXTRACT_DIR" "${OUTPUT_ISO}" "$ISO_PATH" "$EFI_IMG_FILE"  || exit 1
rmdir "$EXTRACT_DIR"
