...
# ------------------------------------------------------------------------
# Functions related to reading an INI file
# ------------------------------------------------------------------------
#
...
INI_ALLOW_DEREFERENCE=no
INI_NEED_SECTIONS=no
...
function ini_read() {
    ...
    do
        ...
        then
            ...
            # ------------------- Safety check start ---------------------
            local safe=yes
            ini_check_safety $section $var $val || safe=no
            if [ "$safe" != "yes" ]; then
                if [ -n "$section" ]; then
...
function ini_process_cmdline() {
    ...
    if [ "$SHOW_WARNINGS" = "yes" ]; then
        cat -- "$ini_path" | remove_comments | ini_read "$ini_file"
    else
        cat -- "$ini_path" | remove_comments | ini_read "$ini_file" 2>/dev/null
    fi
}
